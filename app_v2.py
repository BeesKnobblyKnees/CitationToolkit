...
/* Section dividers injected before items 1 (Citation Repair), 4 (Audit), 7 (Finalise) */
section[data-testid="stSidebar"] .stRadio > div > label:nth-child(2)::before,
section[data-testid="stSidebar"] .stRadio > div > label:nth-child(5)::before,
section[data-testid="stSidebar"] .stRadio > div > label:nth-child(8)::before {
...
button[kind="primary"], .stButton > button[kind="primary"] {
  ...
  box-shadow: 0 1px 4px rgba(61,90,110,0.22) !important;
  transition: background 0.15s, box-shadow 0.15s !important;
}
button[kind="primary"]:hover {
  background: #2c4455 !important;
  box-shadow: 0 2px 8px rgba(61,90,110,0.35) !important;
}
...
with st.sidebar:
    ...
    if "_active_tool_idx" not in st.session_state:
        st.session_state["_active_tool_idx"] = 1  # default: App 02

    # Single radio — no grouping split — so every item is always re-clickable.
    # Section labels are injected as disabled-looking markdown between items via CSS.
    # index=None means no item is pre-committed; the session state index tracks active.
    _chosen = st.radio(
        "",
        _all_tools,
        index=st.session_state["_active_tool_idx"],
        label_visibility="collapsed",
        key="_nav_single",
    )
    if _chosen:
        st.session_state["_active_tool_idx"] = _all_tools.index(_chosen)
    tool = _sel_map.get(_chosen or _all_tools[st.session_state["_active_tool_idx"]],
