...
with st.sidebar:
    ...
    def _nav_btn(label, key, app_val, desc=""):
        ...
        # Use a real st.button — styled via CSS to look like a nav item
        # The button IS the clickable element; we style it to match the design
        clicked = st.button(
            label,
            key=key,
        ...
        if clicked:
            ...
            st.rerun()
        # Inject per-button active styling immediately after
        active_style = (
            f"section[data-testid=\"stSidebar\"] div[data-testid=\"stButton\"] "
            f"button[kind=\"secondary\"][aria-label=\"{label}\"] {{ "
            f"background: rgba(139,26,26,0.09) !important; "
            f"border-left: 2px solid #8b1a1a !important; "
            f"font-weight: 600 !important; color: #1a1208 !important; }}"
        ) if active else ""
        if active_style:
            st.markdown(f"<style>{active_style}</style>", unsafe_allow_html=True)
