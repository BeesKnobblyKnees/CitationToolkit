...
# ─────────────────────────────────────────────────────────────────────────────
# APP 4 — DOCUMENT MERGER (core logic)
# ─────────────────────────────────────────────────────────────────────────────

def extract_citation_fields(raw_xml):
    """
    Extracts all EN.CITE field structures from document XML.
    Returns a dict keyed by a position fingerprint -> field data.
    """
    fields = []
    # Match complete field structures: begin ... instrText ... separate ... result ... end
    field_pattern = re.compile(
        r'<w:fldChar w:fldCharType="begin".*?<w:fldChar w:fldCharType="end"/>',
        re.DOTALL
    )
    instr_pattern = re.compile(r'ADDIN EN\.CITE')
    
    for i, m in enumerate(field_pattern.finditer(raw_xml)):
        field_xml = m.group(0)
        if instr_pattern.search(field_xml):
            fields.append({
                'index': i,
                'start': m.start(),
                'end':   m.end(),
...
def analyze_merge_damage(merged_bytes):
    """
    Analyzes a post-merge document for citation field damage.
    Detects: truncated fields, duplicate fields, fields missing instrText data,
    fields where accept-changes corrupted the field boundary XML.
    """
    import html as html_mod
    ...
    # Count EN.CITE fields
    total_en   = raw.count('ADDIN EN.CITE')
    with_data  = len(re.findall(r'&lt;EndNote&gt;', raw))
    empty_cite = len(re.findall(r'<w:instrText[^>]*> ADDIN EN\.CITE </w:instrText>', raw))

    # Count fldData (backup storage)
    fld_data_count = raw.count('<w:fldData')

    # Check for unmatched field markers (sign of corruption)
    balanced = (begins == separates == ends)

    # Check for track changes remnants
    has_tracked = '<w:ins ' in raw or '<w:del ' in raw
    ins_count   = raw.count('<w:ins ')
    del_count   = raw.count('<w:del ')

    # Find citation fields inside tracked change elements (dangerous)
    cite_in_ins = len(re.findall(r'<w:ins[^>]*>.*?ADDIN EN\.CITE.*?</w:ins>', raw, re.DOTALL))
    cite_in_del = len(re.findall(r'<w:del[^>]*>.*?ADDIN EN\.CITE.*?</w:del>', raw, re.DOTALL))

    # Collect all db-ids present
    db_id_pat = re.compile(r'&lt;key[^&]*db-id=&quot;([^&]+)&quot;')
    db_ids     = list(set(db_id_pat.findall(raw)))
    ...
    return {
        ...
        'fld_data_count': fld_data_count,
        'has_tracked':    has_tracked,
        'ins_count':      ins_count,
        'del_count':      del_count,
        'cite_in_ins':    cite_in_ins,
        'cite_in_del':    cite_in_del,
        'db_ids':         db_ids,
    }


def safe_accept_tracked_changes(raw_xml):
    """
    Accepts tracked changes in the document XML while preserving citation fields.
    
    Strategy:
    1. Accept <w:ins> insertions (keep content) UNLESS they contain EN.CITE fields
    2. Remove <w:del> deletions (discard content) UNLESS adjacent to EN.CITE fields  
    3. Citation fields that are inside <w:ins> blocks are extracted and re-inserted cleanly
    4. Citation fields inside <w:del> blocks are preserved (not deleted)
    
    Returns (fixed_xml, stats_dict)
    """
    stats = {'ins_accepted': 0, 'del_removed': 0, 'citations_rescued': 0, 'citations_preserved': 0}

    # Step 1: Rescue citations trapped inside <w:del> blocks
    # These should NOT be deleted - the citation was there before the merge
    def rescue_from_del(m):
        del_content = m.group(0)
        if 'ADDIN EN.CITE' in del_content:
            # Extract just the citation field runs, discard the rest
            field_pattern = re.compile(
                r'<w:r[^>]*>.*?</w:r>',
                re.DOTALL
            )
            cite_runs = []
            for run_m in field_pattern.finditer(del_content):
                run = run_m.group(0)
                if ('fldChar' in run or 'instrText' in run or 
                    'fldData' in run or 'ADDIN EN' in run):
                    # Convert delText back to regular text
                    run = run.replace('<w:delText', '<w:t').replace('</w:delText>', '</w:t>')
                    cite_runs.append(run)
            if cite_runs:
                stats['citations_rescued'] += 1
                return ''.join(cite_runs)
        # Normal deletion - remove it
        stats['del_removed'] += 1
        return ''

    del_pattern = re.compile(r'<w:del[^>]*>[\s\S]*?</w:del>', re.DOTALL)
    fixed = del_pattern.sub(rescue_from_del, raw_xml)

    # Step 2: Accept <w:ins> insertions
    def accept_ins(m):
        ins_content = m.group(0)
        # Extract inner content (everything between <w:ins...> and </w:ins>)
        inner = re.sub(r'^<w:ins[^>]*>', '', ins_content)
        inner = re.sub(r'</w:ins>$', '', inner)
        stats['ins_accepted'] += 1
        return inner

    ins_pattern = re.compile(r'<w:ins[^>]*>[\s\S]*?</w:ins>', re.DOTALL)
    fixed = ins_pattern.sub(accept_ins, fixed)

    # Step 3: Fix any citations that are now structurally broken
    # (field begin/separate/end counts should now match)
    begins    = fixed.count('fldCharType="begin"')
    separates = fixed.count('fldCharType="separate"')
    ends      = fixed.count('fldCharType="end"')
    stats['field_balance'] = f'{begins}/{separates}/{ends}'
    stats['balanced_after'] = (begins == separates == ends)
...
def repair_post_merge_citations(merged_bytes, original_bytes=None):
    """
    Full repair pipeline for a post-merge document:
    1. Analyze damage
    2. Accept tracked changes safely (preserving citations)
    3. Restore broken citation fields from fldData backup
    4. Optionally compare against original to recover lost citations
    Returns (fixed_bytes, report)
    """
    ...
    report = {
        'analysis':         analysis,
        'steps_applied':    [],
        'citations_before': analysis['with_data'],
        'tracked_changes':  analysis['has_tracked'],
        'ins_count':        analysis['ins_count'],
        'del_count':        analysis['del_count'],
    }

    fixed = raw

    # Step A: Accept tracked changes safely if any exist
    if analysis['has_tracked']:
        fixed, tc_stats = safe_accept_tracked_changes(fixed)
        report['steps_applied'].append('track_changes_accepted')
        report['tc_stats'] = tc_stats

    # Step B: Restore broken citation fields from fldData
    if analysis['empty_cite'] > 0 or analysis['broken_fields'] if 'broken_fields' in analysis else False:
        pattern = re.compile(
            r'(<w:instrText[^>]*>) ADDIN EN\.CITE (</w:instrText>)'
            r'([\s\S]{0,2000}?)'
            r'<w:fldData[^>]*>([\s\S+?]+?)</w:fldData>',
        ...
        def restore_fn(m):
            ...
            try:
                ...
                dec = b64mod.b64decode(b64+'='*pad).decode('utf-8',errors='replace')
        ...
        fixed = pattern.sub(restore_fn, fixed)
        fixed = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', fixed)
        if fixes > 0:
            report['steps_applied'].append(f'restored_{fixes}_broken_fields')

    # Step C: Compare against original to detect lost citations
    if original_bytes:
        with zipfile.ZipFile(io.BytesIO(original_bytes)) as z:
            orig_raw = z.read('word/document.xml').decode('utf-8')
        
        orig_rec_nums = set(re.findall(r'&lt;RecNum&gt;(\d+)&lt;/RecNum&gt;', orig_raw))
        merged_rec_nums = set(re.findall(r'&lt;RecNum&gt;(\d+)&lt;/RecNum&gt;', fixed))
        lost = orig_rec_nums - merged_rec_nums
        
        report['orig_citation_count'] = len(orig_rec_nums)
        report['merged_citation_count'] = len(merged_rec_nums)
        report['lost_rec_nums'] = list(lost)
        
        if lost:
            report['steps_applied'].append(f'{len(lost)}_citations_lost_in_merge')

    # Count citations after all fixes
    report['citations_after'] = len(re.findall(r'&lt;EndNote&gt;', fixed))

    # Write output
    fixed = re.sub(r'[\x00]', '', fixed)  # Final null byte sweep
    all_files['word/document.xml'] = fixed.encode('utf-8')
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zout:
        for name, data in all_files.items():
...
with st.sidebar:
    ...
    find missing/extra refs<br><br>
    <b style="color:#4fc3f7">App 4</b> — Merge documents safely,<br>
    preserve citation field codes
    </div>
    """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown("""
    <div class="instruction-box">
    <b>When to use this:</b> You have an original Word document with linked EndNote citations,
    and an edited version (with track changes, possibly a different library or placeholders).
    Merging them normally breaks the citation field codes. This tool merges the text edits
    safely while preserving all citation fields from the original.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="step-card active">
        <div class="step-header">
            ...
            <span class="step-title">Upload your documents</span>
        </div>
        <div class="step-desc">
            Upload both versions. The <b>original</b> has the correct EndNote citations linked
            to your library. The <b>edited version</b> has the text changes you want to keep.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Original document** — has correct EndNote citations")
        orig_file = st.file_uploader("Original .docx", type=["docx"], key="merge_orig")
    with col2:
        st.markdown("**Edited version** — has text edits / track changes")
        edit_file = st.file_uploader("Edited .docx", type=["docx"], key="merge_edit")
    ...
    <div class="step-card active">
        <div class="step-header">
            ...
            <span class="step-title">Choose what to do with the edited version</span>
        </div>
    </div>
    """, unsafe_allow_html=True)

    strategy = st.radio("Merge strategy", [
        "Fix post-merge document — I already merged in Word but citations broke",
        "Safe merge — combine the two documents, preserving all citations",
    ], label_visibility="visible")

    has_enl = st.toggle("I have the KAROL .enl library to check for missing refs", value=False)
    enl_file_merge = None
    if has_enl:
        enl_file_merge = st.file_uploader("KAROL .enl library", type=["enl"], key="merge_enl")
    ...
    <div class="step-card active">
        <div class="step-header">
            ...
            <span class="step-title">Analyze and repair</span>
        </div>
    </div>
    """, unsafe_allow_html=True)

    run_disabled = not (
        (orig_file and strategy.startswith("Fix")) or
        (orig_file and edit_file and strategy.startswith("Safe"))
    )

    if st.button("Analyze & repair citations", type="primary", disabled=run_disabled):
        if strategy.startswith("Fix"):
            # User already merged — fix the broken result
            with st.spinner("Analyzing citation damage..."):
                merged_bytes = orig_file.read()
                analysis     = analyze_merge_damage(merged_bytes)
            ...
            col2.metric("Working citations",        analysis['with_data'])
            col3.metric("Broken (empty) fields",   analysis['empty_cite'])
            col4.metric("Track changes remaining", analysis['ins_count'] + analysis['del_count'])

            if not analysis['balanced']:
                st.warning(f"⚠ Unbalanced field markers detected "
                           f"(begin:{analysis['begins']} / "
                           f"separate:{analysis['separates']} / "
                           f"end:{analysis['ends']}) — "
                           f"some citation fields were split during merge.")
            else:
                st.success("✓ Citation field boundaries are intact.")

            if analysis['cite_in_del']:
                st.warning(f"⚠ {analysis['cite_in_del']} citation(s) found inside deleted "
                           f"text blocks — these will be rescued automatically.")

            if analysis['db_ids']:
                st.info(f"Library fingerprints found: {len(analysis['db_ids'])} "
                        f"({'multiple libraries — possible merge conflict' if len(analysis['db_ids']) > 1 else 'single library'})")

            # Run repair
            with st.spinner("Repairing..."):
                orig_bytes_for_compare = None  # no original to compare against in this mode
                fixed_bytes, report    = repair_post_merge_citations(
                    merged_bytes,
                    orig_bytes_for_compare
                )
            ...
            col3.metric("Steps applied",    len(report['steps_applied']))

            if report['steps_applied']:
                st.markdown("**Actions taken:**")
                step_labels = {
                    'track_changes_accepted': '✓ Tracked changes accepted safely',
                }
                for step in report['steps_applied']:
                    if step in step_labels:
                        st.markdown(f"- {step_labels[step]}")
                    elif step.startswith('restored_'):
                        n = step.split('_')[1]
                        st.markdown(f"- ✓ {n} broken citation field(s) restored from backup data")
                    elif 'lost' in step:
                        n = step.split('_')[0]
                        st.markdown(f"- ⚠ {n} citation(s) lost during merge — see below")

            if report.get('lost_rec_nums'):
                st.warning(f"⚠ {len(report['lost_rec_nums'])} citation(s) from original are missing in merged document.")
                st.markdown("These RecNums were in the original but not found after merge:")
                for rn in report['lost_rec_nums']:
                    st.markdown(f"- RecNum **{rn}**")

            # Download
            doc_name = orig_file.name
            st.download_button(
                "⬇ Download repaired document (.docx)",
                data=fixed_bytes,
                file_name=Path(doc_name).stem + "_citations_repaired.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                type="primary"
            )
            ...
            1. Open the repaired document in Word with KAROL connected<br>
            2. <b>EndNote tab → Update Citations and Bibliography</b><br>
            3. If any citations are still unlinked, use App 2's "Remap traveling library citations"
            tool to fix remaining RecNum mismatches.
            </div>
            """, unsafe_allow_html=True)

        else:
            # Safe merge mode — keep citations from original, apply text edits from edited version
            with st.spinner("Reading documents..."):
                orig_bytes = orig_file.read()
                edit_bytes = edit_file.read()

                # Analyze both
                orig_analysis = analyze_merge_damage(orig_bytes)
                edit_analysis = analyze_merge_damage(edit_bytes)

            st.markdown("### Document analysis")
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("**Original**")
                st.metric("Citation fields", orig_analysis['total_en'])
                st.metric("Working citations", orig_analysis['with_data'])
                if orig_analysis['has_tracked']:
                    st.warning(f"Has {orig_analysis['ins_count']} tracked changes")
            with col2:
                st.markdown("**Edited version**")
                st.metric("Citation fields", edit_analysis['total_en'])
                if edit_analysis['has_tracked']:
                    st.info(f"Has {edit_analysis['ins_count']} insertions, {edit_analysis['del_count']} deletions")
                else:
                    st.success("No tracked changes")

            st.markdown("""
            <div class="instruction-box">
            <b>Safe merge process:</b><br><br>
            The safest way to merge when citations are involved is a two-step process:<br><br>
            <b>Step A</b> — Use Word's built-in Compare:<br>
            &nbsp;&nbsp;Review tab → Compare → Compare Documents<br>
            &nbsp;&nbsp;Original = your linked document, Revised = the edited version<br><br>
            <b>Step B</b> — Download the fixed original below, then upload the merged result back
            into this tool using the <b>"Fix post-merge document"</b> option above to repair
            any citation fields that got damaged during the compare.<br><br>
            This two-step approach is more reliable than attempting a fully automated merge
            of Word XML, which can produce unpredictable field code interactions.
            </div>
            """, unsafe_allow_html=True)

            # Still repair the original as a clean starting point
            with st.spinner("Preparing clean base document..."):
                fixed_bytes, report = repair_post_merge_citations(orig_bytes)

            st.markdown("### Prepared base document")
            st.success(f"Original document cleaned: {report['citations_after']} citations intact.")

            col1, col2 = st.columns(2)
            with col1:
                st.download_button(
                    "⬇ Download cleaned original",
                    data=fixed_bytes,
                    file_name=Path(orig_file.name).stem + "_cleaned.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    type="primary",
                    help="Use this as the 'Original document' in Word's Compare tool"
                )
            with col2:
                st.download_button(
                    "⬇ Download edited version",
                    data=edit_bytes,
                    file_name=Path(edit_file.name).stem + "_edited.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    help="Use this as the 'Revised document' in Word's Compare tool"
                )

            st.markdown("""
            <div class="instruction-box" style="border-color:#ffb300">
            <b>Word Compare instructions:</b><br>
            1. Open Word → <b>Review tab → Compare → Compare Documents</b><br>
            2. Original document = the <b>cleaned original</b> you just downloaded<br>
            3. Revised document = the <b>edited version</b><br>
            4. Click OK — Word creates a new merged document with track changes<br>
            5. Review the changes — accept text edits but <b>reject any changes that touch citation fields</b><br>
            6. Save the merged document<br>
            7. Come back here and use <b>"Fix post-merge document"</b> to repair any remaining citation damage
            </div>
            """, unsafe_allow_html=True)
