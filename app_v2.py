...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Crimson+Pro:ital,wght@0,300;0,400;0,600;1,300;1,400&family=JetBrains+Mono:wght@400;500&family=DM+Sans:wght@300;400;500;600&display=swap');
...
h2 { font-size: 2rem; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem; margin-bottom: 1.5rem; }
code, pre, .mono {
...
section[data-testid="stSidebar"] * { color: var(--ink-dim) !important; }
...
.nav-item.active {
  background: linear-gradient(90deg, rgba(91,141,238,0.18) 0%, transparent 100%);
...
.nav-item.active .nav-label { color: var(--ink) !important; font-weight: 500; }
.nav-tag {
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.55rem;
...
.card-green  { border-left: 3px solid var(--green); background: #0d1910; }
.card-red    { border-left: 3px solid var(--red);   background: #1a0e0e; }
.card-amber  { border-left: 3px solid var(--amber); background: #1a1508; }
...
.instruction-box b { color: var(--ink); font-weight: 500; }
...
.instruction-box ul { margin: 0.4rem 0 0; padding-left: 1.2rem; }
.instruction-box li { margin-bottom: 0.3rem; }
...
.metric-chip.good .m-val { color: var(--green); }
.metric-chip.bad  .m-val { color: var(--red); }
.metric-chip.warn .m-val { color: var(--amber); }
...
.ref-item.error   { border-color: var(--red);   background: rgba(224,92,92,0.08);  color: #c8a0a0; }
.ref-item.warning { border-color: var(--amber);  background: rgba(232,168,42,0.08); color: #c8b080; }
.ref-item.ok      { border-color: var(--green);  background: rgba(76,175,130,0.08); color: #90c0a0; }

/* ── Step / stage indicators ─────────────────────────────────────────── */
.stage-pill {
...
.stage-pill.active { background: rgba(91,141,238,0.2); color: var(--accent); border: 1px solid rgba(91,141,238,0.4); }
.stage-pill.done   { background: rgba(76,175,130,0.2); color: var(--green);  border: 1px solid rgba(76,175,130,0.4); }
.stage-pill.wait   { background: var(--surface2);      color: var(--ink-dim); border: 1px solid var(--border); }
...
.match-card.accepted { border-left-color: var(--green); background: #0d1910; }
.match-card.skipped  { border-left-color: var(--red);   background: #150d0d; }
.match-sentence { font-size: 0.86rem; color: var(--ink-dim); font-style: italic; margin-bottom: 0.5rem; line-height: 1.65; }
.match-marker   { font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; background: var(--surface2); color: var(--accent); padding: 2px 7px; border-radius: 3px; display: inline-block; margin-bottom: 6px; }
.score-pill     { font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; padding: 2px 8px; border-radius: 3px; }
.score-high { background: rgba(76,175,130,0.2);  color: var(--green); }
.score-mid  { background: rgba(232,168,42,0.2);  color: var(--amber); }
.score-low  { background: rgba(224,92,92,0.2);   color: var(--red); }
...
div[data-testid="stMetric"] label { color: var(--ink-dim) !important; font-size: 0.78rem !important; }
...
button[kind="primary"] {
  ...
  box-shadow: 0 2px 8px rgba(91,141,238,0.35) !important;
  transition: box-shadow 0.2s, transform 0.1s !important;
}
button[kind="primary"]:hover {
  box-shadow: 0 4px 16px rgba(91,141,238,0.55) !important;
...
div[data-testid="stAlert"] { border-radius: var(--radius) !important; border-left-width: 3px !important; }
...
/* Progress bar */
div[data-testid="stProgress"] > div { background: var(--surface2) !important; border-radius: 4px !important; }
div[data-testid="stProgress"] > div > div { background: var(--accent) !important; border-radius: 4px !important; }
/* Tabs */
div[data-testid="stTabs"] button { font-family: 'DM Sans', sans-serif !important; font-size: 0.85rem !important; }
...
/* Caption */
.stCaption { color: var(--ink-dim) !important; font-size: 0.78rem !important; }
/* Sidebar radio hidden — we use custom nav */
section[data-testid="stSidebar"] div[data-testid="stRadio"] { display: none; }
</style>
...
with st.sidebar:
    ...
    <div style="padding: 1.4rem 1rem 0.8rem;">
      <div class="sidebar-logo">Citation Toolkit</div>
      <div class="sidebar-sub">Tachdjian's Pediatric Orthopaedics</div>
    </div>
    ''', unsafe_allow_html=True)

    # Hidden radio for actual routing (invisible — controlled by buttons below)
    tool = st.radio("Tool", [
        "App 1 — PubMed Search",
        "App 2 — Broken Citation Fixer",
        "App 3 — Document Merger",
        "App 4 — Citation Repair",
        "App 5 — Bibliography Auditor",
        "App 6 — Reference Comparator",
        "App 7 — Citation Renumbering",
        "App 8 — Figure Inventory",
        "App 9 — Document Health Check",
        "App 10 — Batch Rename",
        "App 11 — RecNum Inspector",
    ], label_visibility="collapsed", key="tool_radio")

    # Styled nav built on top — clicking sets the radio via session state
    def _nav(num, label, desc, key):
        active = tool == key
        active_cls = "active" if active else ""
        st.markdown(f'''
        <div class="nav-item {active_cls}" style="margin-bottom:2px;">
          <span class="nav-num">{num}</span>
          <div>
            <div class="nav-label">{label}</div>
            <div style="font-size:0.69rem;color:var(--ink-faint);line-height:1.2;margin-top:1px;">{desc}</div>
          </div>
        </div>''', unsafe_allow_html=True)

    st.markdown('<div class="sidebar-section">Search & Import</div>', unsafe_allow_html=True)
    _nav("01", "PubMed Search",        "Find & export refs",       "App 1 — PubMed Search")

    st.markdown('<div class="sidebar-section">Citation Repair</div>', unsafe_allow_html=True)
    _nav("02", "Broken Citation Fixer","Fix stuck/broken fields",  "App 2 — Broken Citation Fixer")
    _nav("03", "Document Merger",      "Safe merge with citations","App 3 — Document Merger")
    _nav("04", "Citation Repair",      "Fill placeholder refs",    "App 4 — Citation Repair")

    st.markdown('<div class="sidebar-section">Audit & Compare</div>', unsafe_allow_html=True)
    _nav("05", "Bibliography Auditor", "Cross-ref vs published",   "App 5 — Bibliography Auditor")
    _nav("06", "Reference Comparator", "Compare two ref lists",    "App 6 — Reference Comparator")
    _nav("11", "RecNum Inspector",     "Look up library RecNums",  "App 11 — RecNum Inspector")

    st.markdown('<div class="sidebar-section">Finalise</div>', unsafe_allow_html=True)
    _nav("07", "Citation Renumbering", "Reorder numerically",      "App 7 — Citation Renumbering")
    _nav("08", "Figure Inventory",     "Check figure names",       "App 8 — Figure Inventory")
    _nav("09", "Document Health Check","QA scan + repair",         "App 9 — Document Health Check")
    _nav("10", "Batch Rename",         "Find & replace from Excel","App 10 — Batch Rename")
...
if tool == "App 2 — Broken Citation Fixer":
    st.markdown('<span class="stage-pill active">App 02</span>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<span class="stage-pill active">App 04</span>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<span class="stage-pill active">App 03</span>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="stage-pill active">App 07</span>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<span class="stage-pill active">App 10</span>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="stage-pill active">App 05</span>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="stage-pill active">App 09</span>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="stage-pill active">App 11</span>', unsafe_allow_html=True)
