...
with st.sidebar:
    ...
    # Each group uses index=None so nothing is pre-highlighted.
    # Clicking any item fires a rerun; we detect the change via prev_ tracking.
    tool = st.radio("", ["01 · PubMed Search"],
                    index=None, label_visibility="collapsed", key="_nav_s1")
    st.markdown('<div class="sidebar-rule">Citation Repair</div>', unsafe_allow_html=True)
    tool2 = st.radio("", ["02 · Broken Citation Fixer","03 · Document Merger","04 · Citation Repair"],
                     index=None, label_visibility="collapsed", key="_nav_s2")
    st.markdown('<div class="sidebar-rule">Audit & Compare</div>', unsafe_allow_html=True)
    tool3 = st.radio("", ["05 · Bibliography Auditor","06 · Reference Comparator","11 · RecNum Inspector"],
                     index=None, label_visibility="collapsed", key="_nav_s3")
    st.markdown('<div class="sidebar-rule">Finalise</div>', unsafe_allow_html=True)
    tool4 = st.radio("", ["07 · Citation Renumbering","08 · Figure Inventory",
                           "09 · Document Health Check","10 · Batch Rename"],
                     index=None, label_visibility="collapsed", key="_nav_s4")

    for _r, _k in [(tool,"_nav_s1"),(tool2,"_nav_s2"),(tool3,"_nav_s3"),(tool4,"_nav_s4")]:
        if _r is not None and _r != st.session_state.get("_prev_"+_k):
            st.session_state["_last_tool"] = _r
            st.session_state["_prev_"+_k] = _r

    tool = _sel_map.get(st.session_state["_last_tool"], "App 2 — Broken Citation Fixer")
