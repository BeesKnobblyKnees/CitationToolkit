...
/* Hide the raw Streamlit buttons — only the styled div previews show */
...
with st.sidebar:
    ...
    if "tool" not in st.session_state:
        st.session_state["tool"] = "App 2 — Broken Citation Fixer"

    def _nav_btn(label, key, app_val, desc=""):
        active = st.session_state["tool"] == app_val
        bg     = "rgba(139,26,26,0.09)" if active else "transparent"
        border = "2px solid #8b1a1a"    if active else "2px solid transparent"
        color  = "#1a1208"
        st.markdown(f"""
        <div style="background:{bg};border-left:{border};border-radius:3px;
                    padding:6px 10px 6px 10px;margin-bottom:2px;cursor:pointer;">
          <div style="font-size:0.82rem;font-weight:{'600' if active else '400'};
                      color:{color};font-family:'Source Sans 3',sans-serif;
                      line-height:1.3;">{label}</div>
          {"<div style=\"font-size:0.69rem;color:#8a7a65;margin-top:1px;font-family:'Source Sans 3',sans-serif;\">" + desc + "</div>" if desc else ""}
        </div>""", unsafe_allow_html=True)
        if st.button(label, key=key, use_container_width=True):
            st.session_state["tool"] = app_val
            st.rerun()

    st.markdown('<div class="sidebar-rule">Search &amp; Import</div>', unsafe_allow_html=True)
    _nav_btn("01 · PubMed Search",        "_t01", "App 1 — PubMed Search",        "Find & export refs")

    st.markdown('<div class="sidebar-rule">Citation Repair</div>', unsafe_allow_html=True)
    _nav_btn("02 · Broken Citation Fixer","_t02", "App 2 — Broken Citation Fixer","Fix stuck/broken fields")
    _nav_btn("03 · Document Merger",      "_t03", "App 3 — Document Merger",      "Safe merge")
    _nav_btn("04 · Citation Repair",      "_t04", "App 4 — Citation Repair",      "Fill placeholders")

    st.markdown('<div class="sidebar-rule">Audit &amp; Compare</div>', unsafe_allow_html=True)
    _nav_btn("05 · Bibliography Auditor", "_t05", "App 5 — Bibliography Auditor", "Cross-ref vs published")
    _nav_btn("06 · Reference Comparator","_t06", "App 6 — Reference Comparator", "Compare two lists")
    _nav_btn("11 · RecNum Inspector",     "_t11", "App 11 — RecNum Inspector",    "Look up RecNums")

    st.markdown('<div class="sidebar-rule">Finalise</div>', unsafe_allow_html=True)
    _nav_btn("07 · Citation Renumbering","_t07", "App 7 — Citation Renumbering",  "Reorder numerically")
    _nav_btn("08 · Figure Inventory",    "_t08", "App 8 — Figure Inventory",      "Check figure names")
    _nav_btn("09 · Document Health Check","_t09","App 9 — Document Health Check", "QA scan + repair")
    _nav_btn("10 · Batch Rename",        "_t10", "App 10 — Batch Rename",         "Find & replace")
